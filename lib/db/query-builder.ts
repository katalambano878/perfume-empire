...
interface EmbedSpec {
  ...
  inner?: boolean; // PostgREST !inner — require a related row
  select: ParsedSelect;
}
...
function uuidSafeLeft(col: string, value: any): string {
  return uuidSafeQualified(ident(col), col, value);
}

function uuidSafeQualified(qualified: string, colName: string, value: any): string {
  const looksUuidCol = colName === "id" || colName.endsWith("_id");
  if (
    looksUuidCol &&
    typeof value === "string" &&
    value.length > 0 &&
    ...
    if (parenIdx !== -1) {
      ...
      if (rhs.includes("!")) {
        ...
        if (hint === "inner" || hint === "left") {
          ...
          const fkColumn = hint.replace(/_fkey$/, "").split("_").length
            ? deriveFkFromConstraint(hint)
            : undefined;
          result.embeds.push({
            alias,
            table: tableName.trim(),
...
class QueryBuilder implements PromiseLike<{ data: any; error: any; count: number | null }> {
  ...
  // ---- WHERE construction ---------------------------------------------------
  private buildWhere(params: any[], extra: string[] = []): string {
    ...
    for (const f of this.filters) {
      if (f.kind === "cmp") {
        ...
        if (!f.values || f.values.length === 0) {
          ...
          clauses.push(this.relatedPredicate(f.col!, (q) => `${q} IN (${ph.join(",")})`));
        }
        ...
        clauses.push(
          this.relatedPredicate(
            f.col!,
            (q) => `(${q} IS NULL OR ${q} NOT IN (${ph.join(",")}))`
          )
        );
      } else if (f.kind === "is") {
        ...
        clauses.push(f.sql);
        if (f.params) params.push(...f.params);
      }
    }
    return clauses.length ? `WHERE ${clauses.join(" AND ")}` : "";
  }

  /** `categories.slug` → EXISTS subquery; plain columns stay on this table. */
  private relatedPredicate(col: string, predicate: (qualified: string) => string): string {
    if (!col.includes(".")) return predicate(ident(col));
    const parts = col.split(".");
    if (parts.length !== 2 || !PG_IDENT.test(parts[0]) || !PG_IDENT.test(parts[1])) {
      throw new Error(`Unsafe SQL identifier: ${col}`);
    }
    const [rel, relatedCol] = parts;
    const relQ = ident(rel);
    const thisQ = ident(this.table);
    const qualified = `${relQ}.${ident(relatedCol)}`;
    const fwd = (FK_MAP[this.table] || []).find((e) => e.foreignTable === rel);
    if (fwd) {
      return `EXISTS (SELECT 1 FROM ${relQ} WHERE ${relQ}.${ident("id")} = ${thisQ}.${ident(fwd.column)} AND ${predicate(qualified)})`;
    }
    const rev = (FK_MAP[rel] || []).find((e) => e.foreignTable === this.table);
    if (rev) {
      return `EXISTS (SELECT 1 FROM ${relQ} WHERE ${relQ}.${ident(rev.column)} = ${thisQ}.${ident("id")} AND ${predicate(qualified)})`;
    }
    throw new Error(`Unknown embed filter ${col} on ${this.table}`);
  }

  private relatedExists(rel: string): string {
    if (!PG_IDENT.test(rel)) throw new Error(`Unsafe SQL identifier: ${rel}`);
    const relQ = ident(rel);
    const thisQ = ident(this.table);
    const fwd = (FK_MAP[this.table] || []).find((e) => e.foreignTable === rel);
    if (fwd) {
      return `EXISTS (SELECT 1 FROM ${relQ} WHERE ${relQ}.${ident("id")} = ${thisQ}.${ident(fwd.column)})`;
    }
    const rev = (FK_MAP[rel] || []).find((e) => e.foreignTable === this.table);
    if (rev) {
      return `EXISTS (SELECT 1 FROM ${relQ} WHERE ${relQ}.${ident(rev.column)} = ${thisQ}.${ident("id")})`;
    }
    throw new Error(`Unknown !inner embed ${rel} on ${this.table}`);
  }

  private innerJoinClauses(parsed: ParsedSelect): string[] {
    return parsed.embeds.filter((e) => e.inner).map((e) => this.relatedExists(e.table));
  }

  private isClause(col: string, value: any): string {
    let pred: (q: string) => string;
    if (value === null || value === "null") pred = (q) => `${q} IS NULL`;
    else if (value === true || value === "true") pred = (q) => `${q} IS TRUE`;
    else if (value === false || value === "false") pred = (q) => `${q} IS FALSE`;
    else pred = (q) => `${q} IS NOT DISTINCT FROM ${value}`;
    return this.relatedPredicate(col, pred);
  }
  ...
  private cmpClause(col: string, op: string, value: any, params: any[]): string {
    ...
    params.push(value);
    const clause = this.relatedPredicate(col, (q) => {
      const left = col.includes(".")
        ? uuidSafeQualified(q, col.split(".").pop()!, value)
        : uuidSafeLeft(col, value);
      return `${left} ${o} $${params.length}`;
    });
  ...
  private orTerm(seg: string, params: any[]): string {
    ...
    if (rest.startsWith("in.(") && rest.endsWith(")")) {
      ...
      return this.relatedPredicate(col, (q) => `${q} IN (${placeholders.join(", ")})`);
    }
    ...
    params.push(val);
    return this.relatedPredicate(col, (q) => {
      const left = col.includes(".")
        ? uuidSafeQualified(q, col.split(".").pop()!, val)
        : uuidSafeLeft(col, val);
      return `${left} ${o} $${params.length}`;
    });
  }
  ...
  async exec(): Promise<{ data: any; error: any; count: number | null }> {
    try {
      ...
      if (this.action === "select") {
        ...
        const params: any[] = [];
        const where = this.buildWhere(params, this.innerJoinClauses(parsed));
        if (this.wantCount) {
      ...
      if (fk) {
        ...
        if (isCountOnly) {
          ...
          if (parentIds.length) {
            ...
            const res = await pool.query(
              `SELECT ${ident(fkCol)} AS _fk, count(*)::int AS count FROM ${ident(embedTable)} WHERE ${ident(fkCol)} IN (${ph}) GROUP BY ${ident(fkCol)}`,
              parentIds
            );
            for (const r of res.rows) counts.set(r._fk, r.count);
          }
          for (const r of rows) {

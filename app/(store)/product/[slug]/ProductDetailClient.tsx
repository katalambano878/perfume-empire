...
export default function ProductDetailClient({ slug }: { slug: string }) {
    ...
    return (
      <div className="min-h-screen bg-white py-20 flex justify-center items-center">
        <div className="text-center">
          ...
          <Link href="/shop" className="text-brand hover:underline">Return to Shop</Link>
        </div>
      </div>
    );
  }
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: product.category, url: absoluteUrl(`/shop?category=${product.category.toLowerCase().replace(/\s+/g, '-')}`) },
    { name: product.name, url: absoluteUrl(`/product/${slug}`) },
  ]);
        ...
        <section className="py-8 bg-gray-50 border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <nav className="flex items-center space-x-2 text-sm flex-wrap gap-y-2">
              ...
              <Link href="/shop" className="text-gray-600 hover:text-brand transition-colors">Shop</Link>
              <i className="ri-arrow-right-s-line text-gray-400"></i>
              <Link href="#" className="text-gray-600 hover:text-brand transition-colors">{product.category}</Link>
              <i className="ri-arrow-right-s-line text-gray-400"></i>
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    {product.images.map((image: string, index: number) => (
                      <button
                        ...
                        className={`relative aspect-square rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${selectedImage === index ? 'border-brand shadow-md' : 'border-gray-200 hover:border-gray-300'
                          }`}
                      >
                        <Image
                          src={image}
              ...
              <div>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center border-2 border-gray-200 hover:border-brand rounded-full transition-colors cursor-pointer"
                  >
                ...
                {hasVariants && product.colors.length > 0 && (
                  <div className="mb-6">
                    <label className="block font-semibold text-gray-900 mb-3">
                      Color: {selectedColor ? (
                        <span className="text-brand font-normal">{selectedColor}</span>
                      ) : (
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-2.5 rounded-full border-2 font-medium transition-all whitespace-nowrap cursor-pointer flex items-center gap-2 ${isSelected
                              ? 'border-brand bg-cream text-brand shadow-sm'
                              : isOutOfStock
                      ...
                      <div className="mb-8">
                        <label className="block font-semibold text-gray-900 mb-3">
                          Variant: {selectedVariant ? (
                            <span className="text-brand font-normal">{selectedVariant.name} · GH₵{selectedVariant.price?.toFixed(2)}</span>
                          ) : (
                          ...
                          {product.variants.map((variant: any) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className={`text-xs mt-0.5 ${isSelected ? 'text-brand' : 'text-gray-500'}`}>
                    ...
                    <div className="flex flex-col">
                      ...
                      {activeStock > 10 && (
                        <span className="text-gray-600 font-medium text-sm">
                          <i className="ri-checkbox-circle-line mr-1 text-brand"></i>
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                  <button
                    ...
                    className={`flex-1 bg-gray-900 hover:bg-brand text-white py-4 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2 text-lg whitespace-nowrap cursor-pointer ${(activeStock === 0 || needsVariantSelection || needsColorSelection) ? 'opacity-50 cursor-not-allowed' : ''}`}
                  ...
                  {activeStock > 0 && !needsVariantSelection && !needsColorSelection && (
                    <button
                      ...
                      className="sm:w-auto bg-brand hover:bg-brand-dark text-white px-8 py-4 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer"
                    >
                      Buy Now
                    </button>
                  )}
                ...
                <div className="border-t border-gray-200 pt-6 space-y-4">
                  <div className="flex items-center text-gray-700">
                    <i className="ri-store-2-line text-xl text-brand mr-3"></i>
                    <span>Free store pickup available</span>
                  </div>
                  <div className="flex items-center text-gray-700">
                    <i className="ri-arrow-left-right-line text-xl text-brand mr-3"></i>
                    <span>30-day easy returns and exchanges</span>
                  </div>
                  <div className="flex items-center text-gray-700">
                    <i className="ri-shield-check-line text-xl text-brand mr-3"></i>
                    <span>Secure payment & buyer protection</span>
                  </div>
                  {product.sku && (
                    <div className="flex items-center text-gray-700">
                      <i className="ri-barcode-line text-xl text-brand mr-3"></i>
                ...
                <ul className="grid md:grid-cols-2 gap-4">
                  {product.features.map((feature: string, index: number) => (
                    <li key={index} className="flex items-start">
                      <i className="ri-checkbox-circle-fill text-brand text-xl mr-3 mt-1"></i>

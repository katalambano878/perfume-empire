...
export default function FreeShippingBar({ currentAmount, threshold = 200 }: FreeShippingBarProps) {
  ...
  return (
    <div className={`rounded-lg p-4 mb-4 ${
      isQualified 
        ? 'bg-gradient-to-r from-cream0 to-brand-light text-white' 
        : 'bg-gradient-to-r from-gold-light to-cream border-2 border-gold-light'
    }`}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2">
          <div className={`w-8 h-8 flex items-center justify-center rounded-full ${
            isQualified ? 'bg-white/20' : 'bg-gold'
          }`}>
      ...
      <div className="relative">
        <div className={`w-full h-3 rounded-full overflow-hidden ${
          ...
          <div
            className={`h-full transition-all duration-500 ${
              isQualified 
                ? 'bg-white' 
                : 'bg-gradient-to-r from-gold to-brand-accent'
            }`}
        ...
        {isQualified && (
          <div className="absolute right-0 top-1/2 -translate-y-1/2 -mr-1">
            <div className="w-6 h-6 flex items-center justify-center bg-white rounded-full text-brand">

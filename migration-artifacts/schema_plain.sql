-- ============================================================================
-- Plain PostgreSQL schema — The Perfume Empire
-- Apply once on empty database (includes auth bootstrap).
-- Source: supabase/migrations/* adapted for fleet plain-Postgres pattern.
-- No live customer data. RLS policies stripped (app uses table owner via node-pg).
-- ============================================================================
...
-- Plain Postgres auth stub (GoTrue-compatible subset)
...
-- RLS enabled without Supabase role policies (app connects as DB owner).
...
-- Contact form submissions (no RLS — app-layer auth)
...
-- Ensure auth signup trigger exists
DO $$
BEGIN
  IF NOT EXISTS (
